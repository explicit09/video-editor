...
struct TestMediaGenerator {
    ...
    func makeVideoWithAudio(
        ...
        try await writeVideoFile(to: videoURL, duration: duration, color: color, width: width, height: height, fps: fps)
    ...
    private func writeVideoFile(
        ...
        let pixelBuffer = try createPixelBuffer(width: width, height: height, color: color)
    ...
    private func createPixelBuffer(
        width: Int, height: Int, color: (r: UInt8, g: UInt8, b: UInt8)
...
struct CompositionStructuralTests {
    ...
    @Test("Split clip produces two video tracks with correct segments")
    func splitClipStructure() async throws {
        ...
        let videoTracks = comp.tracks(withMediaType: .video)
        #expect(videoTracks.count == 2, "Each clip gets its own video track")

        // First clip: target 0-1s, source 0-1s
        let seg0 = (videoTracks[0].segments ?? []).first { !$0.isEmpty }
        #expect(seg0 != nil, "First track should have a non-empty segment")
        #expect(abs((seg0?.timeMapping.target.start.seconds ?? -1) - 0.0) < 0.05)
        #expect(abs((seg0?.timeMapping.target.duration.seconds ?? 0) - 1.0) < 0.1)

        // Second clip: target 1-2s, source 1-2s
        let seg1 = (videoTracks[1].segments ?? []).first { !$0.isEmpty }
        #expect(seg1 != nil, "Second track should have a non-empty segment")
        #expect(abs((seg1?.timeMapping.target.start.seconds ?? -1) - 1.0) < 0.05)

        // Audio: should be 1 shared track covering both clips
        let audioTracks = comp.tracks(withMediaType: .audio)
