...
struct WaveformView: View {
    ...
    var body: some View {
        Canvas { context, size in
            // Recompute samples only when width or source data changes
            let maxSamples = max(Int(size.width / 3), 40)
            let sampled: [Float]
            if abs(size.width - cachedWidth) > 2 || cachedSamples.isEmpty {
                sampled = sampledAmplitudes(maxSamples: maxSamples)
                DispatchQueue.main.async {
                    cachedSamples = sampled
                    cachedWidth = size.width
                }
            } else {
                sampled = cachedSamples
            }
